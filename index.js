...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        app.post('/api/notes', authenticate, async (req, res) => {
          ...
          if (!title || !text) {
            return res.status(400).json({ message: 'Título y texto son requeridos' });
          }
        
          try {
            // Insertar la nota en la base de datos
            await db.query('INSERT INTO notes (userId, title, text, category, public, image) VALUES (?, ?, ?, ?, ?, ?)', [req.user.id, title, text, category, public, image]);
...
const authenticate = (req, res, next) => {
  ...
  app.get('/api/notes/:id', authenticate, async (req, res) => {
    ...
    const note = await db.query('SELECT * FROM notes WHERE userId = ? AND id = ?', [req.user.id, req.params.id]);
